...
export interface DailyPoints {
    ...
    desktopFinalPoints?: number;  // 桌面端完成后的积分
    mobileFinalPoints?: number;   // 移动端完成后的最终积分
}
...
export function loadConfig(): Config {
    ...
    try {
        // 尝试多个可能的配置文件路径
        const possiblePaths = [
            path.join(__dirname, '../', 'config.json'),  // 开发环境
        ...
        if (!configContent) {
            throw new Error(`配置文件未找到，尝试的路径: ${possiblePaths.join(', ')}`);
        }
        
        configCache = JSON.parse(configContent);
        console.log(`✅ 配置文件加载成功: ${usedPath}`);
        return configCache;
    } catch (error) {
        const errorMessage = error instanceof Error ? error.message : String(error);
