...
class BrowserFunc {
    ...
    async getAppEarnablePoints(data, accessToken) {
        try {
            ...
            const userDataResponse = (await this.bot.axios.request(userDataRequest)).data;
    ...
    async loadInCheerio(page) {
        const html = await page.content();
        return (0, cheerio_1.load)(html);
    }
    async getPunchCardActivity(page, activity) {
        let selector = '';
        try {
            const html = await page.content();
            const $ = (0, cheerio_1.load)(html);
    ...
    async closeBrowser(browser, email) {
        try {
            await (0, Load_1.saveSessionData)(this.bot.config.sessionPath, browser, email, this.bot.isMobile);
            await this.bot.utils.wait(2000);
            await browser.close();
