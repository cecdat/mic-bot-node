...
exports.Login = exports.LoginStatusCode = void 0;
const crypto = __importStar(require("crypto"));
const fs_1 = __importDefault(require("fs"));
const path_1 = __importDefault(require("path"));
const Load_1 = require("../util/Load");
exports.LoginStatusCode = {
    Success: 0,
    PasswordError: 1,
    Locked: 2,
...
class Login {
    ...
    async login(page, email, password) {
        ...
        try {
            ...
            if (isLoggedIn) {
                ...
                await this.bot.sendStatusUpdate(platformType, true, exports.LoginStatusCode.Success, '会话有效');
                await this.checkAccountLocked(page, email);
            }
            else {
            ...
            let code = exports.LoginStatusCode.GenericFailure;
            if (errorMessage.includes('密码不正确'))
                code = exports.LoginStatusCode.PasswordError;
            if (errorMessage.includes('此账户已被锁定'))
                code = exports.LoginStatusCode.Locked;
    ...
    async execLogin(page, email, password) {
        ...
        try {
            ...
            await this.bot.sendStatusUpdate(platformType, true, exports.LoginStatusCode.Success, '登录成功');
    ...
    async handle2FA(page) {
        const platformType = this.bot.isMobile ? 'mobile' : 'pc';
        try {
            await this.bot.sendStatusUpdate(platformType, false, exports.LoginStatusCode.AuthorizationRequired, '需要2FA/授权');
    ...
    async get2FACode(page) {
        ...
        try {
            ...
            const code = await codeHandle.jsonValue();
    ...
    async authAppVerification(page, numberToPress) {
        if (!numberToPress) {
            ...
            const accountEmail = await page.evaluate(() => document.querySelector('#bannerText')?.innerText || '未知账号');
    ...
    async checkLoggedIn(page, email) {
    ...
    async dismissLoginMessages(page, email) {
        // 处理"保持登录状态"弹窗
    ...
    async handleVerifyEmailPage(page, email) {
        const platformType = this.bot.isMobile ? 'mobile' : 'pc';
        // 同时检测页面标题和h1标签
        ...
        if (pageTitle.includes("验证你的电子邮件") || await verifyEmailTitle.isVisible({ timeout: 2000 })) {
            ...
            await this.bot.sendStatusUpdate(platformType, false, exports.LoginStatusCode.VerificationRequired, '需要邮件验证');
            // 尝试找到并点击"使用密码"选项（支持多种元素类型和文本变体）
            // 1. 首先尝试直接匹配包含'使用密码'的所有可见元素
            // 扩大匹配范围，包含更多可能的文本变体和元素类型
            const usePasswordOptions = page.locator('div:has-text("已收到代码？") + div >> *, '
                + 'div:has-text("Already have a code?") + div >> *, '
                + '*:has-text("使用密码"), *:has-text("Use your password"), '
                + '*:has-text("密码登录"), *:has-text("Password login"), '
                + '*:has-text("账号密码登录"), *:has-text("Sign in with password")').filter({ visible: true });
            // 增加超时时间并添加详细日志
     ...
     * 处理其他类型的验证页面
     */
    async handleOtherVerificationPages(page, email) {
        const platformType = this.bot.isMobile ? 'mobile' : 'pc';
        // 检测是否有密码错误
        ...
        if (await invalidPassword.isVisible({ timeout: 1000 })) {
            ...
            await this.bot.sendStatusUpdate(platformType, false, exports.LoginStatusCode.PasswordError, '密码不正确');
        ...
        if (await securityVerificationTitle.isVisible({ timeout: 1000 })) {
            ...
            await this.bot.sendStatusUpdate(platformType, false, exports.LoginStatusCode.VerificationRequired, '需要安全验证');
            // 尝试找到并点击"使用其他方式"链接
        ...
        if (await accountRecoveryTitle.isVisible({ timeout: 1000 })) {
            ...
            await this.bot.sendStatusUpdate(platformType, false, exports.LoginStatusCode.Locked, '账户需要恢复');
        }
        // 检测是否有验证码输入页面
        ...
        if (await captchaInput.isVisible({ timeout: 1000 })) {
            ...
            await this.bot.sendStatusUpdate(platformType, false, exports.LoginStatusCode.VerificationRequired, '需要输入验证码');
            // 在调试模式下保存快照
    ...
    async saveSnapshot(page, email, filename) {
        // 提取文件名（不含扩展名）
        const baseFilename = filename.replace(/\.html$/, '');
        this.bot.log(this.bot.isMobile, '调试模式', `[${email}] 正在保存页面快照...`, 'warn');
        try {
            await this.bot.utils.wait(2000);
            const sessionDir = path_1.default.join(__dirname, '..', '..', this.bot.config.sessionPath, email);
            if (!fs_1.default.existsSync(sessionDir)) {
                fs_1.default.mkdirSync(sessionDir, { recursive: true });
            }
            // 保存HTML格式（保持原有功能）
            const htmlContent = await page.content();
            const htmlSnapshotPath = path_1.default.join(sessionDir, `${baseFilename}.html`);
            fs_1.default.writeFileSync(htmlSnapshotPath, htmlContent);
            this.bot.log(this.bot.isMobile, '调试模式', `HTML快照已成功保存到: ${htmlSnapshotPath}`, 'log', 'green');
            // 保存图片格式（新增功能）
            const imageSnapshotPath = path_1.default.join(sessionDir, `${baseFilename}.png`);
    ...
    async checkAccountLocked(page, email) {
        await this.bot.utils.wait(2000);
