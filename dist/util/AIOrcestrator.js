...
class AIOrcestrator {
    ...
    getAllIncompleteTasks(data) {
        ...
        if (data.promotionalItem && !data.promotionalItem.complete && data.promotionalItem.pointProgressMax > 0) {
            allTasks.push(data.promotionalItem);
        }
        (0, Logger_1.log)('main', 'AI指挥部', `共收集到 ${allTasks.length} 个可执行的任务。`);
        return allTasks;
    }
    /**
     * 调用AI，获取最优的任务执行计划
     ...
     */
    async getTaskExecutionPlan(tasks) {
        (0, Logger_1.log)('main', 'AI指挥部', '正在请求AI制定最优执行计划...');
        // **********************************************************************
        // ** 【AI集成点】 **
        // **********************************************************************
        const simulatedPlan = this.utils.shuffleArray(tasks);
        (0, Logger_1.log)('main', 'AI指挥部', 'AI已返回作战计划！将按以下顺序执行:');
        // [核心修改] 在打印日志时，调用翻译函数
        simulatedPlan.forEach((task, index) => {
            const translatedTitle = translateTitle(task.title); // 获取翻译后的标题
            (0, Logger_1.log)('main', 'AI作战计划', `  ${index + 1}. [${task.pointProgressMax}分] ${translatedTitle} (类型: ${task.promotionType})`);
        });
        return simulatedPlan;
    }
}
exports.aiOrchestrator = new AIOrcestrator();
//# sourceMappingURL=AIOrcestrator.js.map